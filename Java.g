...
modifiers  returns [String value] //modified by Ann M
    ...
    )* {if(modifiers=="") modifiers="null "; $value = modifiers;}
    ;
...
normalClassDeclaration // modified by Ann M & Mike M
    :   a=modifiers  'class' IDENTIFIER {classInfo.headerInfo(modifiers); modifiers = ""; classInfo.setName($IDENTIFIER.text);}
        (typeParameters
        )?
        ...
        classBody
    ;
typeParameters returns [String value] // modified by Ann M
    :   '<'	{myString = "<";}
            e=typeParameter  {myString += $e.value;}
            (',' f=typeParameter {myString += ", "; myString += $f.value;}
            )*			{$value = myString;}
        '>'
    ;
typeParameter returns [String value]
...
typeBound 
    ...
    ;
enumDeclaration 
    :   modifiers {classInfo.headerInfo(modifiers);}
        ('enum'
        ) 
        IDENTIFIER
        ('implements' typeList
...
memberDecl // modified by Ann M
    ...
    |    methodDeclaration {classInfo.addMethod(methodInfo);}
    |    classDeclaration
    |    interfaceDeclaration
    ;
methodDeclaration returns [String value] //modified by Ann M
    :
        ...
        IDENTIFIER {classInfo.addMethodCall($IDENTIFIER.text); methodInfo.setName($IDENTIFIER.text); methodInfo.setReturnType("constructor");}
        e=formalParameters {isFormalParameter = true; myString = ""; myString2 = ""; myString3 = ""; if($e.value != "")methodInfo.addParameter($e.value); isFormalParameter = true;}
        ...
        '}' {classInfo.addToAllMethodCalls(classInfo.getMethodCalls()); classInfo.emptyMethodCalls();}
    |   modifiers {methodInfo = new MethodInfo(); modifiers = ""; String mods[] = $modifiers.value.split(" "); for(int i = 0; i < mods.length; ++i){methodInfo.addModifier(mods[i]);}}
        (typeParameters {}
        )?
        ...
        IDENTIFIER {classInfo.addMethodCall($IDENTIFIER.text); methodInfo.setName($IDENTIFIER.text);}
        f=formalParameters  {isFormalParameter = true; myString = ""; myString2 = ""; myString3 = ""; if($f.value != "")methodInfo.addParameter($f.value); isFormalParameter = true;}
        ('[' ']'
        )*
        ('throws' qualifiedNameList
        )?            
        (        
            block 
        |   ';' 
        ) {{classInfo.addToAllMethodCalls(classInfo.getMethodCalls()); classInfo.emptyMethodCalls();} $value = myMethodString;}
    ;
fieldDeclaration returns [String value] // modified by: Ann M
    :   modifiers {if($modifiers.value != null) myDataString = $modifiers.value; } 
        type	  {isDataMember = true; if($type.value != null) myDataString += $type.value.trim(); if(isClass && isDataMember && !$type.value.equals("String")){classInfo.addAggregate($type.value); isClass = false; isDataMember = false;}}
        ...
        ';' {}
    ;
variableDeclarator returns [String value] // modified by: Ann M
    :   IDENTIFIER	{$value = $IDENTIFIER.text;}
        ('[' ']'
        )*
        ('=' variableInitializer
...
interfaceFieldDeclaration 
    :   modifiers type variableDeclarator
        ...
        ';'
    ;
type returns [String value]	// modified by Ann M
    :   classOrInterfaceType {$value = $classOrInterfaceType.value; isDataMember = false;}
        ('[' ']'
        )*
    |   primitiveType	     {$value = $primitiveType.value; isClass = false;}
        ('[' ']'	     {$value = $value + "[]";}
        )*
    ;
classOrInterfaceType returns [String value]	// modified by Ann M
    :   e=IDENTIFIER	{isClass = true; $value = $e.text;} 
        (f=typeArguments	
        )?			{isClass = true; classString = $e.text; if($f.value != null) classString += $f.value; $value = classString;}
        ('.' IDENTIFIER
            (typeArguments
            )?
        )*		
    ;
primitiveType  returns [String value] // modified by Ann M
    ...
    |   'double'	{$value = "double";}
    ; 				
typeArguments returns [String value] // modified by Ann M
    :   '<' e=typeArgument {myString2 = "<"; if($e.value != null) myString2 += $e.value;}
        (',' f=typeArgument {myString2 += ", "; if($f.value != null) myString2 += $f.value;}
        )* 
        '>' {myString2 += ">"; $value = myString2;}
    ;
typeArgument returns [String value]	// modified by Ann M
...
formalParameters returns [String value] // modified by Ann M
    :   '(' {}
        (formalParameterDecls {isFormalParameter = true; myString = ""; if($formalParameterDecls.value != null) myString += $formalParameterDecls.value; classInfo.addFormalParameters($formalParameterDecls.value);}
        )? 
        ')'{$value = myString;}
    ;
...
formalParameterDecls returns [String value] // modified by Ann M
    ...
    |   (g=normalParameterDecl	{myString2 = ""; if($g.value != null) myString2 += $g.value;}
        ...
        ellipsisParameterDecl
    ;
normalParameterDecl returns [String value] // modified by Ann M
    :   variableModifiers type IDENTIFIER {myString3 = ""; if($variableModifiers.value != null) myString3 = $variableModifiers.value; 
    								myString3 += $type.value; myString3 += " "; myString3 += $IDENTIFIER.text; if(isClass && !$type.value.equals("String")){classInfo.addAssociation($type.value); isClass = false;}}
        ('[' ']' {myString3 += "[]";}
        )* {$value = myString3;}
    ;
ellipsisParameterDecl 
...
system_call //Modified by Amish M
	:	
	a='System.out.println' '(' STRINGLITERAL ')'  {classInfo.addMethodCall($a.text);}
	|| a='System.out.println' '(' function_call ')' {classInfo.addMethodCall($a.text);}
	|| a='System.out.println' '(' expression ')' {classInfo.addMethodCall($a.text);}
	;


multiple_calls 
	:	
	( '.' b=IDENTIFIER {multipleCallsString+="."+$b.text;}
	)*
	;

function_call //Modified by Amish M
	: a=IDENTIFIER multiple_calls '(' functionParameters ')' {classInfo.addMethodCall($a.text + multipleCallsString); multipleCallsString="";}
	|| a=('super.' || 'this.') b=IDENTIFIER '(' functionParameters ')'  {classInfo.addMethodCall($a.text + $b.text);}
	 ;
...
localVariableDeclaration returns [String value]
    :   variableModifiers type {localVarString = ""; if($variableModifiers.value != null ){localVarString += $variableModifiers.value; localVarString += " "; } if($type.value != null){localVarString += $type.value; localVarString += " ";} }
        e=variableDeclarator	{if($e.value != null ) localVarString += $e.value;}
        (',' f=variableDeclarator {localVarString += "\n"; if($variableModifiers.value != null ){localVarString += $variableModifiers.value; localVarString += " "; } if($type.value != null){localVarString += $type.value; localVarString += " ";} if($f.value != null ) localVarString += $f.value; }
...
creator //modified by Amish M
    :   'new' nonWildcardTypeArguments et=type classCreatorRest 
    |   'new' et=type classCreatorRest 
        |   arrayCreator
    ;
